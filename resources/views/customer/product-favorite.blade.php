...
$favourites = [
    ...
    [
        ...
        "tags"   => "WALNUT • MODERN",
        "status" => "In Stock",
        "img"    => "https://images.unsplash.com/photo-1577146333195-6c679b526634?w=600"
    ],
    [
        "name"   => "Aura Sofa Sectional",
        "price"  => "7,200",
        "tags"   => "FABRIC • MINIMALIST",
        "status" => "Low Stock",
    ...
    [
        ...
        "price"  => "1,890",
        "tags"   => "STONE • ARCHITECTURAL",
        "status" => "In Stock",
        "img"    => "https://images.unsplash.com/photo-1612196808214-b8e1d6145a8c?w=600"
    ],
    [
        ...
        "tags"   => "OAK • MINIMALIST",
        "status" => "Out of Stock",
        "img"    => "https://images.unsplash.com/photo-1505693419148-ad30354642cd?w=600"
    ],
    [
        "name"   => "Coda Sideboard",
        "price"  => "3,650",
...
<head>
    ...
    <title>Product Favorite — <?= $site_name ?></title>
...
<main class="flex-grow flex content-container w-full bg-white">
    ...
    <aside class="w-72 border-r border-gray-50 p-10 bg-gray-50/20 shrink-0">
        <div class="text-center mb-10">
            ...
            <h3 class="font-bold text-lg text-gray-900">Julian Voss</h3>
    ...
    <div class="flex-grow p-12 overflow-y-auto">
        <div class="max-w-4xl">
            ...
            <div id="view-grid" class="grid grid-cols-2 xl:grid-cols-3 gap-x-5 gap-y-8">
                ...
                <div class="fav-card group cursor-pointer fade-up" data-name="<?= htmlspecialchars($item['name']) ?>">
                    ...
                    <div class="relative aspect-[4/3] bg-gray-50 rounded-xl overflow-hidden mb-3">
                        ...
                        <!-- Remove btn -->
                        <button class="btn-remove absolute top-3 right-3 w-8 h-8 bg-white rounded-full flex items-center justify-center text-gray-400 hover:text-red-400 shadow-sm transition-colors"
                                onclick="removeFav(this)">
                            <i class="fa-solid fa-xmark text-xs"></i>
                        </button>

                        <!-- Hover actions -->
                        <div class="card-actions absolute inset-x-3 bottom-3 flex gap-2">
                            <button class="flex-1 bg-white text-gray-900 text-[9px] font-black uppercase tracking-widest py-2.5 rounded-lg hover:bg-primary hover:text-white transition-all">
                                View Product
                            </button>
                            <button class="w-10 h-10 bg-white rounded-lg flex items-center justify-center hover:bg-primary hover:text-white transition-all text-gray-600 shrink-0 <?= $item['status'] === 'Out of Stock' ? 'opacity-40 cursor-not-allowed' : '' ?>"
                    ...
                    <!-- Info -->
                    <div class="px-0.5">
                        <p class="text-[9px] font-black uppercase tracking-[0.18em] text-gray-300 mb-0.5"><?= $item['tags'] ?></p>
                        <div class="flex items-center justify-between gap-2">
                            <h3 class="text-[13px] font-semibold text-gray-900 leading-snug group-hover:text-primary transition-colors truncate"><?= $item['name'] ?></h3>
                            <span class="text-[12px] font-bold text-gray-700 shrink-0">$<?= $item['price'] ?></span>
                        </div>
                    </div>
            ...
            <div id="view-list" class="hidden space-y-3">
                ...
                <div class="fav-card group flex items-center gap-5 p-4 bg-gray-50/40 rounded-2xl border border-gray-100 hover:border-primary/20 hover:bg-gray-50 transition-all fade-up"
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-[9px] font-black uppercase tracking-[0.18em] text-gray-300 mb-0.5"><?= $item['tags'] ?></p>
                        <h3 class="text-[14px] font-semibold text-gray-900 group-hover:text-primary transition-colors truncate"><?= $item['name'] ?></h3>
                    </div>

                    <!-- Price -->
                    <span class="text-[14px] font-bold text-gray-800 shrink-0">$<?= $item['price'] ?></span>

                    <!-- Actions -->
                    <div class="card-actions flex items-center gap-2 shrink-0">
                        <button class="text-[10px] font-black uppercase tracking-widest border border-gray-200 px-4 py-2 rounded-lg hover:border-primary hover:text-primary transition-all text-gray-500">
                            View
                        </button>
                        <button class="w-9 h-9 bg-primary text-white rounded-lg flex items-center justify-center hover:bg-primary/90 transition-all shrink-0 <?= $item['status'] === 'Out of Stock' ? 'opacity-40 cursor-not-allowed' : '' ?>"
                                <?= $item['status'] === 'Out of Stock' ? 'disabled' : '' ?>>
                            <i class="fa-solid fa-bag-shopping text-xs"></i>
                        </button>
                        <button class="btn-remove w-9 h-9 border border-gray-200 rounded-lg flex items-center justify-center text-gray-400 hover:text-red-400 hover:border-red-200 transition-all"
                                onclick="removeFav(this)">
...
<script>
    ...
    function updateCount() {
        const gridCards = document.querySelectorAll('#view-grid .fav-card').length;
        const listCards = document.querySelectorAll('#view-list .fav-card').length;
        const count     = Math.max(gridCards, listCards);

        document.querySelector('header p').textContent = count + ' saved items';
