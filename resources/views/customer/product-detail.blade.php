...
<head>
    ...
    <style>
        ...
        .thumb.active { border-color: #B5733A; }
        ...
        .review-card { border: 1px solid #EDE8E2; border-radius: 16px; padding: 20px; background: #fff; transition: box-shadow .2s, transform .2s; position: relative; overflow: hidden; }
        .review-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, #B5733A, #E8A96A); opacity: 0; transition: opacity .2s; }
        .review-card:hover { box-shadow: 0 8px 28px rgba(0,0,0,.07); transform: translateY(-2px); }
        .review-card:hover::before { opacity: 1; }
        .review-photo { width: 60px; height: 60px; border-radius: 9px; object-fit: cover; border: 2px solid #EDE8E2; cursor: pointer; transition: border-color .15s, transform .15s; }
        .review-photo:hover { border-color: #B5733A; transform: scale(1.06); }
    </style>
</head>
...
<header class="bg-white border-b border-gray-100 sticky top-0 z-50">
    <div class="content-container flex justify-between items-center py-4 px-6 mx-auto max-w-[1200px]">
        ...
        <nav class="hidden md:flex items-center space-x-10 text-[13px] font-medium text-gray-500 tracking-wide">
            <a href="{{ route('customer.catalog') }}" class="text-primary font-bold transition-all">Collections</a>
...
<div class="content-container px-6 grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-14 pb-20 fade-up">
    ...
    <div class="flex flex-col gap-10">
        <div class="flex gap-3 max-h-[480px]">
            <div class="hidden md:flex flex-col gap-2 w-14 shrink-0">
                ...
                @empty
                <div class="thumb aspect-square rounded-md overflow-hidden border-2 cursor-pointer active border-primary">
            ...
            <div class="flex-1 aspect-[3/4] max-h-[480px] bg-gray-50 rounded-xl overflow-hidden relative group">
                <img id="main-img" src="{{ $product->images->isNotEmpty() ? asset($product->images->first()->img_url) : 'https://via.placeholder.com/800' }}" alt="{{ $product->name }}"
                     class="w-full h-full object-cover">
            </div>
        </div>

        <div id="reviews">
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:20px; padding-bottom:16px; border-bottom:1px solid #EDE8E2;">
                <div>
                    <p style="font-size:10px; font-weight:800; letter-spacing:.18em; text-transform:uppercase; color:#B5733A; margin-bottom:4px;">Customer Reviews</p>
                ...
                <div class="review-card">
                    <div style="display:flex; align-items:flex-start; justify-content:space-between; gap:12px; margin-bottom:12px;">
                        <div style="display:flex; align-items:center; gap:10px;">
                            <img src="https://api.dicebear.com/7.x/avataaars/svg?seed={{ urlencode($r->user->full_name ?? 'User') }}" style="width:38px; height:38px; border-radius:10px; background:#E3DCD6; flex-shrink:0;">
                            <div>
                                <span style="font-size:13px; font-weight:800;">{{ $r->user->full_name ?? 'Anonymous' }}</span>
                                <p style="font-size:9px; color:#A89E97;">{{ $r->created_at->format('M d, Y') }}</p>
                            </div>
                        </div>
                        <div style="flex-shrink:0;">
                            @for($s=1; $s<=5; $s++)
                            <span style="color:{{ $s <= $r->rating ? '#F59E0B' : '#E5E7EB' }}; font-size:11px;">★</span>
                            @endfor
                        </div>
                    </div>
                    <p style="font-size:14px; font-weight:800; margin-bottom:7px; color:#1C1410;">{{ $r->title ?? 'Sangat memuaskan!' }}</p>
                    <p style="font-size:13px; color:#4B4440; line-height:1.72; margin-bottom:12px;">{{ $r->comment }}</p>
                </div>
                @empty
    ...
    <div class="lg:sticky lg:top-24 self-start space-y-7">
        ...
        <div class="space-y-2 pb-6 border-b border-gray-100">
            <span class="text-[9px] font-black uppercase tracking-[0.3em] text-primary">{{ $product->style ?? 'Exclusive' }} Collection</span>
            <h1 class="text-4xl font-bold tracking-tight leading-tight text-gray-900">{{ $product->name }}</h1>
            <div class="flex items-baseline gap-3 mt-2">
                <span class="text-3xl font-light text-primary">Rp {{ number_format($product->price, 0, ',', '.') }}</span>
                @if($product->stock < 5)
                    <span class="text-[10px] font-black uppercase tracking-wider text-red-600 bg-red-50 px-2 py-1 rounded-full">Sisa {{ $product->stock }}!</span>
                @endif
            </div>
        </div>
        ...
        <div class="border-t border-gray-100 pt-6 space-y-4">
            ...
            <div class="flex gap-4">
                <span class="text-[10px] font-black uppercase tracking-wider text-gray-400 w-24 shrink-0 pt-0.5">Category</span>
                <p class="text-[13px] text-gray-700 leading-relaxed">{{ $product->category->name ?? '-' }}</p>
            </div>
            <div class="flex gap-4">
                <span class="text-[10px] font-black uppercase tracking-wider text-gray-400 w-24 shrink-0 pt-0.5">Design Style</span>
...
<section class="bg-gray-50/60 py-16 border-t border-gray-100">
    <div class="content-container px-6">
        <div class="flex items-end justify-between mb-10">
            <div>
                <p class="text-[10px] font-black uppercase tracking-[0.3em] text-primary mb-1">Curated Pairings</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <a href="{{ route('product.detail', $rec->id) }}" class="group">
                ...
                <div class="flex items-center justify-between px-1">
                    <h4 class="font-semibold text-[14px] text-gray-900 group-hover:text-primary transition-colors">{{ $rec->name }}</h4>
                    <span class="text-[13px] font-bold text-gray-700">Rp {{ number_format($rec->price, 0, ',', '.') }}</span>
                </div>
            </a>
            @empty
