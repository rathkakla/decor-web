...
<main id="main-content" class="flex-1 flex flex-col ml-64 sidebar-transition min-h-screen bg-[#F8F6F4]">
    ...
    <div class="flex items-center gap-4 mb-6">
        <form action="{{ route('admin.designer-support') }}" method="GET" class="flex-1 relative">
            @if(request('status'))
                <input type="hidden" name="status" value="{{ request('status') }}">
            @endif
            <i data-feather="search" class="w-4 h-4 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
            <input type="text" name="search" value="{{ request('search') }}" class="w-full pl-10 pr-4 py-3 bg-white border border-gray-100 rounded-xl text-xs font-semibold focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary shadow-sm" placeholder="Cari ID tiket atau nama desainer…">
            <button type="submit" class="hidden"></button>
        </form>
        <div class="flex gap-1 bg-white border border-gray-100 shadow-sm rounded-xl p-1">
            <a href="{{ route('admin.designer-support', ['status' => 'semua']) }}" class="px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-colors {{ $statusFilter == 'semua' ? 'bg-orange-50 text-primary' : 'text-gray-500 hover:bg-gray-50' }}">Semua</a>
            <a href="{{ route('admin.designer-support', ['status' => 'pending']) }}" class="px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-colors {{ $statusFilter == 'pending' ? 'bg-orange-50 text-primary' : 'text-gray-500 hover:bg-gray-50' }}">Open</a>
            <a href="{{ route('admin.designer-support', ['status' => 'replied']) }}" class="px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-colors {{ $statusFilter == 'replied' ? 'bg-orange-50 text-primary' : 'text-gray-500 hover:bg-gray-50' }}">Diproses</a>
            <a href="{{ route('admin.designer-support', ['status' => 'resolved']) }}" class="px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-colors {{ $statusFilter == 'resolved' ? 'bg-orange-50 text-primary' : 'text-gray-500 hover:bg-gray-50' }}">Selesai</a>
        </div>
    </div>
